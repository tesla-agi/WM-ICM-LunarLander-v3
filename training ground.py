...
class WorldModel(nn.Module):
    ...
    def reset(self,seed=None,options=None):
        ...
        with torch.no_grad():
            ...
            self.z_t=mu
        combined=torch.cat([self.z_t,self.prev_action],dim=-1).squeeze(0).numpy()
        return combined,info

    def step(self,action):
        with torch.no_grad():                       #MDN memory update
            a_one_hot=F.one_hot(torch.tensor([action]),num_classes=4).float()
...
def final_eval(model,vae,mdrnn,icm_model,num_episodes=10):
    env=WorldModel(vae,mdrnn,icm_model,eta=0.01)
    rewards=[]
...
if __name__ == "__main__":
    ...
    # Phase 2: Train VAE (with beta fix)
    vae = train_vae(episodes, num_epochs=200)
    ...
    # Phase 6: Train controller with PPO
    model = train_controller(vae, mdrnn, icm_model, total_timesteps=200000, eta=0.01)

    # Evaluate
    print("\n=== Final Evaluation ===")
    rewards = final_eval(model, vae, mdrnn, icm_model, num_episodes=10)
